...
export class ChatService {
  ...
  async fetchResponse(messages: Messages[], selectedEvents: SelectedEventsDto[]) {
    ...
    // design api in a way that matches open ai's expected data map
...
if (!Array.isArray(selectedEvents)) {
  throw new Error("selectedEvents must be an array");
    ...
    const systemPrompt = `You are an educational assistant for a prediction market platform. Your role is to help users understand and analyze market events through conversational Q&A.
...
- If asked for predictions, explain you cannot predict outcomes but can discuss relevant factors
